...
const battle = {
  addResultBattle: async function (req, res) {
    try {
     ...
     if(!isParticipating) throw new Error("You didnt participate in this battle")

    // check if the winner is  participating
     const isWinnerParticipating=req.body.idOpponents.includes(req.body.winner)
     if(!isWinnerParticipating) throw new Error("You didnt participate in this battle")
